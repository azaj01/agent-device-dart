...
| `cd55a6a5` | fix: harden iOS simulator recording cleanup (#575) | skipped | — | All changes are in daemon session state (`record-trace-recording.ts`, new `record-trace-ios-simulator.ts`, `types.ts`). The signal escalation targets a `xcrun simctl io recordVideo` background process managed by the daemon. Dart iOS recording exclusively uses the XCUITest runner (`recordStart`/`recordStop`) — no simctl recordVideo background process exists in the Dart architecture. |
